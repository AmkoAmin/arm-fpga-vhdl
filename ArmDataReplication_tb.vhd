        ...
        process
            procedure CHECK_DRP_OUTPUT(
                ...
                if (DRP_OUTPUT /= next_DRP_OUTPUT) then
                    case DRP_DMAS is
                        when "00"   => STD.TEXTIO.write(TX_LOC, string'("DRP_DMAS = DMAS_BYTE      "));
                        when "01"   => STD.TEXTIO.write(TX_LOC, string'("DRP_DMAS = DMAS_HWORD     "));
                        when "10"   => STD.TEXTIO.write(TX_LOC, string'("DRP_DMAS = DMAS_WORD      "));
                        when others => STD.TEXTIO.write(TX_LOC, string'("DRP_DMAS = DMAS_RESERVED  "));
                    end case;
            ...
            begin
                ...
                if (TX_ERROR = 0) then
                    ...
                    assert (false) report "Simulation nicht erfolgreich (" & integer'image(TX_ERROR) & " Fehler)" severity error;
                end if;
                report SEPARATOR_LINE;
